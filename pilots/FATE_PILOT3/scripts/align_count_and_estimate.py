...
def estimate(output, tmp, p_e=None):
    os.system("module load GCC/9.3.0")
    count_output = f"{output}/count"
...
if __name__ == "__main__":
    ...
    p_e = sys.argv[8]
    snv_csv = sys.argv[9]
    trim_cutoff = int(sys.argv[10])
    do_estimate = bool(int(sys.argv[11]))
    task_id = int(sys.argv[12])
    n_tasks = int(sys.argv[13])
    ...
    for task in cur_tasks:
        ...
        cur_whitelist = meta_dir + f"/RT_{RT}.txt"
