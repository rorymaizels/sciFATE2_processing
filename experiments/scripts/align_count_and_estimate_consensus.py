...
def consensus(output, gtf, tmp):
    alignment = f"{output}/alignment/Aligned.sortedByCoord.out.bam"
    cons_output = f"{output}/consensus"
    cons_tmp = f"{tmp}_O"
    command = f'dynast consensus -g {gtf} --tmp {cons_tmp} --barcode-tag CB --umi-tag UB {alignment} -o {cons_output}'
    os.system(command)


def count(output, gtf, tmp, whitelist, snp_csv=None):
    alignment = f"{output}/consensus/consensus.bam"
...
def estimate(output, tmp, p_e=None):
    ...
    if p_e:
        os.system(
            f"dynast estimate -o {estimate_output} -t 32 --tmp {est_tmp} --p-e {p_e} {count_output}"
        )
    else:
        os.system(
            f"dynast estimate -o {estimate_output} -t 32 --tmp {est_tmp} {count_output}"
        )
...
if __name__ == "__main__":
    ...
    for task in cur_tasks:
        ...
        # 2. run consensus, an edit-preserving alignment method
        cur_tmp = f"{data_dir}/{RT}/tmpO_{RT}_{sample}"
        consensus(cur_output, gtf, cur_tmp)

        # 3. count edits
        cur_tmp = f"{data_dir}/{RT}/tmpC_{RT}_{sample}"
        count(cur_output, gtf, cur_tmp, cur_whitelist, snv_csv)
    if do_estimate:
        # 4. do estimation separately because it takes ages
        for task in cur_tasks:
            sample = sample_df.loc[task]["samples"]
            RT = sample_df.loc[task]["RT"]
            cur_output = f"{data_dir}/{RT}/{sample}"
            cur_tmp = f"{data_dir}/{RT}/tmpE_{RT}_{sample}"
            estimate(cur_output, cur_tmp, p_e) 
