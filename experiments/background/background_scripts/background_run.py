...
def align(sample, RT, PCR, whitelist, output, fastq, tmp, star, trim_cutoff, novaseq_system='S1'):
...
if __name__ == "__main__":
    ...
    do_estimate = bool(int(sys.argv[10]))
    task_id = int(sys.argv[11])
    n_tasks = int(sys.argv[12])

    sample_df = pd.read_csv(meta_dir + f"/{sample_csv_name}_background.csv", index_col=0)
    ...
    for task in cur_tasks:
        ...
        cur_whitelist = meta_dir + f"/RT_{RT}_background.txt"
